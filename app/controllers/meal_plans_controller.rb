class MealPlansController < ApplicationController 
  ...
  def index 
    @current_meal_plan = current_user.meal_plans.where("start_date <= ? AND end_date >= ?", Date.today).first
  ...
  def meal_plan_params 
    params.require(:meal_plan).permit(:start_date, :end_date, meals_atttributes: [:id, :date, :recipe_id])
  end 
end 
